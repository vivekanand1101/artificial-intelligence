...
def forward_diagonal(n, board, player):
    for i in range(n):
        if str(board[(n - 1) * (i + 1)]) != str(player):
            return False
    return True

def backward_diagonal(n, board, player):
    for i in range(n):
        if str(board[(n + 1) * i]) != str(player):
            return False
    return True
...
def main():
    ...
    for i in range(t):
        ...
        print result
        #print is_winning_combination(n, '121211221', '1')
